...
fermis = 1e-3*e*linspace(2,20,numfermis);
%% A vector potential
A_pot_valu = 10;
...
B = 0.00023; %T
for k = 1:Nx
    A_potential(k,:,1) = 5e-10*(k-numpoints/2); % Linear A for uniform Bz
    A_potential(k,:,2) = 0; % 
    A_potential(k,:,3) = 0; %
end
figure
...
for i = 1:Ny
        Hamiltonian_l_minus1(i,i) = -T*exp(1j*(q/hbar)*A_potential(i,10,1)*dx); %sam
...
Lnth = floor(Nx/4);
width = floor(Nx/10);
[Domain_air,Domain_FET] = Domain_Rectangle_Mosfet(Nx,Ny,Nx/4, Nx/2);

Potential_op = 1e4*T .* Domain_air + 0 .* Domain_FET ;
